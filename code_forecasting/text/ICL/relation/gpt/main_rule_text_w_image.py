...
def generate_prompt(row, summary_list_clean, Md52timid, classification, aligned, complementary):
        ...
        for event in events:
            ...
            for image_i in classification[event].keys():
                if classification[event][image_i] == "aligned":
                    ...
                    content_compl_i = "* " + content_compl_i
    ...
    ).format(subject, object, time, focus_facts, related_facts, 
             "\n".join([f"{letter}: {option}" for option, letter  in mapped_options.items()]))
    
    return prompt, prompt_rules, [mapped_options[correct_relation], correct_relation], group_num
...
def main(args):
    ...
    with open(f'{ROOT_PATH}/datasets_forecasting/text/function_json/aligned_gemini_all.json','r') as f:
        aligned = json.load(f)
    with open(f'{ROOT_PATH}/datasets_forecasting/text/function_json/complementary_gemini_all_new.json','r') as f:
    ...
    with tqdm(total=df.shape[0], desc="Event Forecasting") as pbar:
        ...
        with open("./result_ICL_text_w_image_realation.json", "w") as f: 
            json.dump(result, f, ensure_ascii=False, indent=2)
        print('save in ./result_ICL_text_w_image_realation.json')
