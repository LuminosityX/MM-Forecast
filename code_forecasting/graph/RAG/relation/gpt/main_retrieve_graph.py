...
def generate_reasoning_input(row, key_events, related_events,compl_events):
    ...
    input = (
        ...
        ).format(subject, object, time, key_events, related_events, compl_events,
                "\n".join([f"{letter}: {option}" for option, letter  in mapped_options.items()]))
    return input,[mapped_options[correct_relation], correct_relation],mapped_options
...
def main(args):
    ...
    with tqdm(total=df.shape[0], desc="Event Forecasting") as pbar:
        ...
        with open("./result_RAG_graph_with_image_relation.json", "w") as f: 
            json.dump(all_result, f, ensure_ascii=False, indent=2)
        print('save in ./result_RAG_graph_with_image_relation.json')
